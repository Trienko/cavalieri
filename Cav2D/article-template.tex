...
The single Cavalieri integral was originally defined in \cite{ackermann12} without having to resort to vector calculus. In this section we will define the Cavalieri integral using vector calculus. As the reader will undoubtedly realize, if we define the single Cavalieri integral using vector calculus then it becomes trivial to
...
\noindent
Let $R$ denote the region which is bounded by $f(x,y)$, $\mathbf{c}(t)+\left <a,0\right >$, $\mathbf{c}(t)+\left <b,0\right >$ and the $x$-axis. Let the $A_R$ denote 
the area of $R$. We can approximate $A_R$ with:
\begin{equation}
 \label{eq:caval1}
...
$S_f$ via translations of $\mathbf{c}(t)$ along the $xy$ plane and then closing the resulting open faces that form due to the aformentioned connections by employing appropriate surfaces. Let $V_{\mathcal{S}}$ denote the volume of $\mathcal{S}$. We can approximate $V_{\mathcal{S}}$ with:
\begin{equation}
