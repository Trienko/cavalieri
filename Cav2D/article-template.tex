...
\title{Computing volume using irregularly shaped rectangular prisms}
...
Cavalieri's principle assures us that two planar figures or solids that are composed of the same set of indivisibles have equal area or volume \cite{andersen1985,eves1991,jullien2015,malik1984,wildberger2002,william1917,young1985}. Cavalieri's principle 
...
basis and lateral edges that are non-linear curves. Moreover, the four lateral edges of the aforementioned solid also need to be translations of one another. In this paper, we will also take the liberty to refer to an irregular rectangular prism whose base's
length and width both tend to zero as an \emph{irregular rectangular infinitesimal}. \\
...
process of integration. Furthermore, as this paper is mainly meant to be read by undergraduate students as well as the layman who is interested in the theory of integration, we will as far as possible try to steer clear from using too many formal definitions and proofs and rather convey our ideas via worked out examples.\\ 
...
of $h$ is $[a',b']$ (assuming $h$ is monotone increaing). As the partition of $[a,b]$ can be made arbitrarily fine, $h:[a,b]\rightarrow [a',b']$ actually maps every point $x_i^1\in [a,b]$ to a point $x_i^2\in [a',b']$. \\
...
\begin{equation}
 ...
 c_y(t) = f(x_i^1 + c_x(t)).
 \end{equation}
 We can now solve for $t$ in Equation~\eqref{eq:t_func_1d} in terms of $x_i^1$. If we substitute the solution $t(x_i^1)$ so obtained into Equation~\eqref{eq:pat_rel_1d} we obtain 
\begin{equation}
\label{eq:h_func_1d}
...
number of irregularly shaped rectangular infinitesimals.\\
...
Furthermore, let the transformation $T$ map the $xy$--coordinate $(x_i^1,y_j^1)\in R$ (the lower left vertex of $R_{ij}$) at which $\mathbf{c}(t) + \left <x_i^1,y_j^1,0 \right >$ intersects the 
...
%We will now show 
%that we can approximate $V_{\mathcal{S}}$ by summing together the volume of $n$ non-uniform prisms that cicumscribe $\mathcal{S}$ and that as $n\rightarrow \infty$ 
...
Let $\mathcal{S}$ be the solid which is bounded by the surfaces $f(x,y)=\sqrt{-x^2-y^2+4}$, $\sqrt{x}$, $\sqrt{x-1}$, $\sqrt{y}$, $\sqrt{y-1}$ and the $xy$-plane. Solid $\mathcal{S}$ is depicted 
...
\caption{The solid $\mathcal{S}$ which is bounded by the surfaces $f(x,y)=\sqrt{-x^2-y^2+4}$, $\sqrt{x}$, $\sqrt{x-1}$, $\sqrt{y}$, $\sqrt{y-1}$ and the $xy$-plane.}
...
Equation~\eqref{eq:h_2d} and Equation~\eqref{eq:s_2d}. Equation~\eqref{eq:h_2d} and Equation~\eqref{eq:s_2d} in turn requires $t(x_i^1,y_j^1)$, which can be obtained via Equation~\eqref{eq:c_z}.
...
irregularly shaped rectangular infinitesimals. We end the paper with two fully worked out examples. In the first example we compute the 
volume of a prismatoid using the double Cavalieri integral. In the second example we compute the volume of a complex irregular shaped solid which is housed within a sphere using the double Cavalieri integral.
\end{abstract}
...
\bibitem{gonzalez2018} D.~L.~Gonz{\'a}lez, D.~L.~Rabiti, J.~H.~E~Cartwright. 2018. Bonaventura Cavalieri and Bologna. \emph{Math. Intelligencer}. 40(2): 21--29.

\bibitem{grobler19} Grobler,~T.~L. (2019). Visualization of the Riemann-Stieltjes integral. \textit{Coll. Math. J.} 50(3): 198--209.

\bibitem{jullien2015} V.~Jullien (Ed.). (2015). \emph{Seventeenth-Century Indivisibles Revisited}. Boston: Birkh{\"a}user.
