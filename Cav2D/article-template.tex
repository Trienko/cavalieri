...
\noindent
Let $\mathbf{c}(t) = \left <c_x(t),c_y(t) \right>$ be the vector equation of $a(y)-a$. Note that, for every coordinate $(x_i^1,x_i^2) \in [a,b] \times [a',b']$ there exists a unique $t\in\mathbb{R}$ such that
\begin{equation}
\label{eq:pat_rel}
...
c_y(t) = f(x_i^1 + c_x(t)),
\end{equation}
which highlights the fact that $t$ is a function of $x_i^1$. If we substitute $t(x_i)$ into Equation~\ref{eq:pat_rel} we obtain
\begin{equation}
\label{eq:h_func}
...
Equation~\eqref{eq:h_func} implies  
