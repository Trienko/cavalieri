...
vertices lie in two parallel planes. Furthermore, all of its lateral faces are trapezoids or triangles (if viewed from the side). Also note the following: the base of $\mathcal{S}$ is the region 
 $R = [0,4]\times [0,4]$ and that the line of intersection between $2x$ and $y$ is $\mathbf{c}(t) = -\left<\frac{1}{2}t,t,t\right>$. The region $R$ and the line $\mathbf{c}(t)$ are
...
\end{equation}
with $\mathcal{C}=\{\mathbf{c}(t),R\}$. We can, however, only evaluate the above integral if the transformation $T$ is known (see Figure ). We can find $T$ using Equation and 
Equation. To evaluate Equation and Equation we first need to express $t$ in terms of $x_i^1$ and $y_i^1$. If we substitute $f(x,y)$ and $\mathbf{c}(t)$ into Equation we find 
the following expression for $t$:
\begin{equation}
xxx. 
\end{equation}
If we substitute $xxx$ into Equation and Equation we obtain:
\begin{equation}
yyyy 
\end{equation}
and
...
zzzz 
\end{equation}
If we substitute yyyy and zzzz into kkkk we obtain:

We can easily verify that the $V_{\mathcal{S}}$ we computed is correct, since S is a prismatoid. According to the prismatoid volume 
equation $V_{\mathcal{S}}=25.6$. The details of this computation is described in the caption of Figure~\ref{fig:prismatoid}. This result verifies 
the fact that 
