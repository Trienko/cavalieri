...
Galileo held Cavalieri in high esteem. Galileo made the following remark in a reference letter he wrote on Cavalieri's behalf: ``\emph{few, perhaps none, from 
...
\caption{The moon crater Cavalerius. In 1935, this crater was officially named after Cavalieri.}
\label{fig:crater}
\end{figure}
...
area or volume is unknown. Two planar figures or solids that are Cavalieri congruent have equal area or volume. Two planar figures or solids are Cavalieri congruent if they can be decomposed into the same set of parallel ``indivisibles''. An indivisible of 
...
\caption{Illustration of Cavalier's principle in $\mathbb{R}^3$. The volume of the left rectangular prism is equal to the volume of the right non-linear reectangular prism 
due to Cavalieri's principle in $\mathbb{R}^3$.}
\label{fig:cav_principle}
\end{figure}

\noindent
Figure~\ref{fig:cav_principle} depicts a classic rectangular prism or column as well as a \emph{non-linear rectangular prism or column}. The volumes of the two solids in Fig~\ref{fig:cav_principle} are equal due 
to Cavalieri's principle. Note that in this article we use the non-standardized phrase non-linear rectangular prism or column to refer to a solid that has rectangular 
basis and lateral edges that are non-linear curves. Moreover, the four lateral edges of this solid also need to be translations of one another. In this paper, we will also take the liberty to refer to a non-linear rectangular prism whose base's
lenght and widht both tend to zero as a non-linear rectangular infinitesimal. \\

\noindent
Ackermann et al. utilized the idea of using non-rectangular integration strips to perform integration in $\mathbb{R}^2$ and in doing so developed the Cavalieri integral. 
The area of a non-rectangular integration strip is computed via Cavalieri’s principle in $\mathbb{R}^2$. The Cavalieri integral has proven to be very useful as a visualization tool. In this paper we extend the Cavalieri integral into $\mathbb{R}^3$, i.e. we use Cavalieri's principle in $\mathbb{R}^3$ to define the double 
Cavalieri integral. We also show that if a solid can be decomposed into an infinite number of non-linear rectangular infinitesimals then its volume can be computed 
...
The examples we provide at the end of the paper can also be used by an undergraduate Calculus lecturer to improve their students' understanding into the process of integration.   


and show that we can determine the volume of solids that . 
