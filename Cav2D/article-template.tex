...
We can compute $V_{\mathcal{S}}$ as follows:
\begin{equation}
\label{eq:cav_ex2}
V_{\mathcal{S}} = \iint\limits_{\!\mathcal{C}} f(x,y) dA =  \int_a^b\int_c^d f(h(x,y),s(x,y))~dx~dy,
\end{equation}
...
Equation~\eqref{eq:h_2d} and Equation~\eqref{eq:s_2d}. Equation~\eqref{eq:h_2d} and Equation~\eqref{eq:s_2d} in turn requires $t(x_i^1,y_j^2)$, which can be calculated with 
