...
\title{A 2D Geometric Interpretation of the Riemann-Stieltjes Integral}
...
\begin{abstract}
It is desirable to have geometric interpretations for difficult mathematical ideas. In this paper we present an intiutive geometric interpretation of the Riemann-Stieltjes integral. It can be 
summarized as follows: the Riemann-Stieltjes integral can be inerpreted as an infinite sum of initesimally small non-rectangular 
integration strips. This interpretation is surprisingly similar to the geometric intrepretation of the ordinary Riemann integral. 
It is therefore likely that this geometric interpretaion would narrow the logical rift between the Riemann integral and the Riemann-Stieltjes integral (i.e. make the 
Riemann-Stieltjes integral less abstract).
\end{abstract}
...
integral is no exception. Bullock proposed a three dimensional geometric framework with which one can interpret the Riemann-Stieltjes 
integral. Pol proposes to interpret the Riemann-Stieltjes integral as the real distance covered by a moving object, whose speed was 
correctly recorded but at incorrect times. In this paper we present an intiutive two dimensional geometric interpretation of the Riemann-Stieltjes integral.\\

\noindent
?? utilized the idea of using non-rectangular integration strips to perform integration
and in doing so developed the Cavalieri integral. The area of a non-rectangular integration strip is computed via 
Cavalieri's principle.  also showed that a Cavalieri integral can easily be converted to a Riemann-Stieltjes integral. The 
reverse operation was however not addressed therein. In this paper we address this shortcomming. An automatic consequence of being 
able to convert a Riemann-Stieltjes integral to a Cavalieri integral is that a geometric interpretation of the Riemann-Stieltjes
becomes evident. This geometric interpretation is: the Riemann-Stieltjes integral can be interpreted as an infinite sum of infinitesimally small
...
rectangular instead of non-rectangular integration strips. This geometric interpretation has been quite useful while 
theaching the ordinary Riemann integral to undergraduate students. It, therefore, seems likely that undergraduate students would find the non-rectangular geometric interpretation
of the Riemann-Stieltjes integral just as useful. It is also interesting to mention that it has been proposed to first 
teach other integral definitions (which are easier to grasp) before moving on to the Riemann integral. It therefore seems plausible that
students will find it useful to first understand the Cavaleri integral before the Riemann-Stieltjes integral is taught to them.
...
\begin{definition}[Definition of integral]
...
\section{3D Geometric Interpretation}
Bullock proposed one possible geometric interpretation of the Riemann-Stieltjes integral. 
The basic premise behind his approach is to represent $f(x)$ and $g(x)$ as three-dimensional sheets. 
The three dimensional coordinate system which Bullock proposes to realize this representation is depicted in 
...
\noindent
It is interesting to note that Bullock follows a similar route than we do in terms of developing his geometrical framework.
He starts of with quite strict requirements for $f$ ang $g$ and relaxes more and more of these requirements in latter 
examples.

\begin{figure}[htb]
\centering
\includegraphics[width=1.0\textwidth]{3D_final-crop.pdf}
\caption{An example of using Bullocks' geometric framework to interpret the Riemann-Stieltjes integral. In this example 
...
$h(x_i^1) :=$ $\{x_i^2 \in [a' ,b'] | a\circ f(x_i^2) + [x_i^1 - a] = x_i^2$ , $a = a(0)\}$, which we will refer to
...
is defined as $g(x_i^2) := x_i^2 - a \circ f (x_i^2) + a$, which we will refer to as the inverse
...
Otherwise, if $h$ is strictly decreasing, the application of $h$ induces a reversed partition $\mathcal{P}_2 = \{x_n^2, \cdots, x_1^2,x_0^2\}$. In the rest of this paper
we will assume that $h$ is strictly increasing, without any loss of generality.  Let 
...
\noindent
It is however hard to evaluate Eq.~\eqref{eq:caval1} in its present form. Luckily we can transform the Cavalieri sum given in Eq.~\eqref{eq:caval1} into an ordinary Riemann sum
...
\section{2D Geometric Interpretation}
\begin{figure}[htb]
\centering
\includegraphics[width=1\textwidth]{fig13_change}
\caption{A sum-based 2D geometric interpretation of the Riemann-Stieltjes integral. Moreover, the figure depicts the region $R$ (also see Fig.~). The integrator $g$ maps points 
...
\noindent
To improve our understanding of $g$ lets explore a specific example of the Riemann-Stieltjes integral: 
\begin{equation}
\int_{\frac{1}{2}}^1 x~d2x. 
\end{equation}
In this example $g$ maps $[\frac{1}{2},2]$ to $[1,4]$. The mapping between these two intervals is graphically 
...
i.e. a Riemann-Stieltjes integral can be converted into an equivalent Cavalieri integral. Note, that the geometric interpretation presented
in Fig.~\ref{fig:2d_geo} is two dimensional.\\
...
\end{itemize}
Note that the above procedure places an even stricter requirement of $f$, it now also needs to be invertable. Moreover the 
above derivation aslo requires that $a(0)$ exists.
...
thereof is reported in Table~\ref{tab:table2}. The 2D sum-based geometric interpretations of the integrals in Table~\ref{tab:table1}, the $a(y)$ functions provide, are depicted in Figs.~\ref{fig:ex1_sec}--\ref{fig:ex4_sec}.\\
...
computed on the $x$-axis itself, but rather on the line $y=1000$. The fact that the 2D sum-based geometric framework remains useful even if $g(a')\neq a$ is true in general.\\
...
However, the 2D sum-based geometric framework does produce a meaningful result.\\  

\noindent
One of the main limitations of the 2D geometric interpretation we present in this paper is that both $f$ and $g$ need to be invertible.
...
integral whose integrand and integrator are not invertable over its integratrion limits into multiple constittuent Riemann-Stieltjes integrals whose 
integrand and integrator are invertable over the integration limits of the consitutent integrals. For example, say 
$f$ and $g$ are not invertabal over the interval $[a',b']$, but are invertable over the inervals $[a',c']$ and 
$[c',b']$ then we may write:
\begin{equation}
\int_{a'}^{b'}f(x)dg(x) = \int_{a'}^{c'}f(x)dg(x) + \int_{c'}^{b'} f(x)dg(x).
\end{equation}
This allows us to to find a 2D sum-based geometric interpretation of $\int_{a'}^{c'}f(x)dg(x)$ and $\int_{c'}^{b'} f(x)dg(x)$
which we can combine to give a geometric interpretation of $\int_{a'}^{b'}f(x)dg(x)$.
In Fig.~\ref{fig:ex4_sec} we present just such a scenrio. In this example $g(x)$ is not invertible over the interval 
$[0,2]$. It is however invertible over the interval $[0,1]$ and $[1,2]$. The geometric interpretation whic we 
obtain by considering the intervals $[0,1]$ and $[1,2]$ seperately is depicted, respectively, in the 
...
associated with these integrals. The 2D sum-based geometric interpretations of the integrals, the $a(y)$ functions provide, are depicted in Figs.~\ref{fig:ex1_sec}--\ref{fig:ex4_sec}.
The results of evaluaing these integrals are presented in Table~\ref{tab:table2}.
}
\label{tab:table1}
...
evaluate these integrals. The constants $a,b$ and $C$ associated with these integrals are also presented. Moreover, $h_1(x) = 2\sqrt{x+20}+8$, $h_2(x)=\sin(-\cos^{-1}(x+1))$, $h_3(x)=\sqrt{x-C}$, $h_4(x)=1+\sqrt{1-2x}$ and
$h_5(x)=1-\sqrt{1-2x}$.}
\label{tab:table2}
...
\end{table}
 ...
 \caption{The sum-based 2D geometric interpretation of $\int_{1}^3 (2x+8)^3 d[x^2-20]$ (i.e this integral is equal to the 
...
\caption{The sum-based 2D geometric interpretation of $\int_{\frac{-\pi}{2}}^0 \sin{x}~d[\cos{x}-1]$ (i.e this integral integral is equal to the 
...
\caption{The sum-based 2D geometric interpretation of $\int_1^2 \frac{1}{x} d[x^2+C]$ (i.e this integral integral is equal to the 
...
right panel we have the area associated with the integral $\int_0^1 x^2~d[x-\frac{1}{2}x^2]$. This example deomonstrates
...
even when $f$ and $g$ are only piece-wise invertable.}
\label{fig:ex4_sec}
\end{figure}

\section{Conclusion}
A two dimensional geometric interpretation of the Riemann-Stieltjes integral was presented in this paper. We can summarize this 
interpretation as follows: a Riemann-Stieltjes integral can be interpreted as an infinite sum of initesimally small non-rectangular 
integration strips. The hope is that this geometric interpretation will be usefull to instructors while teaching some of the more difficult concepts 
surrounding the Riemann-Stieltjes integral. 
