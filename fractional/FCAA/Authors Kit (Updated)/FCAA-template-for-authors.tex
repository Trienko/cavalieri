...
\noindent
In this article we specifically focus on the geometric interpretation of fractional integrals. To be more specific, we present a novel geometric interpretation of the left-sided Riemann-Liouville fractional integral based at $a'$ \cite{laurent1884}.
...
can be converted into a Riemann-Stieltjes integral. Moreover, Grobler showed that a Riemann-Stieltjes integral can be interpreted as the area which is obtained by summing together 
...
\noindent
We start the paper by reviewing the definition of the Riemann-Stieltjes integral, the Cavalieri integral and the left-sided Riemann-Liouville fractional integral. We then present Podlubny's original geometric framework, followed by our novel geometric framework. We end the paper with two examples and a conclusion.
...
\noindent
Let $R_t^{\alpha}$ denote the region which is bounded by $a_t^{\alpha}(y)$, $b_t^{\alpha}(y)$, $f(t)$ and the $\tau$-axis. Moreover, let $A_{t}^{\alpha}$ denote the area of $R_t^{\alpha}$. Equation~\ref{eq:rl_to_c} implies that when we evaluate $_{a'}I_t^{\alpha}\,f(t)$ we obtain $A_{t}^{\alpha}$. In other words, when we evaluate equation~\ref{eq:rl_to_c} we are actually summing together the area of an infinite number of non-rectangular infinitesimals inscribing $R_t^{\alpha}$. The aforementioned infinitesimals are, however, non-static, since their shape is a function of $\alpha$ and $t$.\\
...
\begin{description}
 ...
 \item[Effect of $t$] Note that the dashed grey-tinted curves in the right-most panel of Figure~\ref{fig:geo1} are translations of $b_{10}^{\sfrac{4}{5}}(y)$. The dashed gray curves and the solid gray-tinted curves in the right-most panel are not equal to one another. The gray-tinted solid curves depicted in each of the panels of Figure~\ref{fig:geo1} are, therefore, not translations of one another. 
\end{description}
...
Note, that for this example $a'=a=0$. The visualization of equation~\eqref{eq:ex2_cav} is depicted in Figure~\ref{fig:geo2}. If we evaluate the integral in equation~\eqref{eq:ex2} for all $\alpha\in \mathsf{A}$ we obtain the curves depicted in Figure~\ref{fig:eval2}. The validity of the geometric interpretation depicted in Figure~\ref{fig:geo2} is corroborated by Figure~\ref{fig:eval2}. 
