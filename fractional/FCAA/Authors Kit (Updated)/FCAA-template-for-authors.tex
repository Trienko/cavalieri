...
integration strip is calculated using Cavalieri's principle. The formal definition of the Cavalieri integral is given in Definition~\ref{def:cav_integral}.
...
readers who are not familiar with the Cavalieri integral to rather first read Section~\ref{??} instead. The reader should only refer back to the definitions 
presented in this sections when things in Section~\ref{??} become unclear. Moreover, the definitions presented below were taken from [?]. Furthermore, none of the theorems in this sections are proved, the proofs are contained in [].
