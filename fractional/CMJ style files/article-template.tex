...
Consider the two fractional integrals:
...
\noindent
$\frac{1}{\Gamma(\alpha)}\int_0^t \tau(t-\tau)^{\alpha-1}~d\tau$ and $\frac{1}{\Gamma(\alpha)}\int_0^t \sqrt{\tau}(t-\tau)^{\alpha-1}~d\tau$.\\
...
The set of functions $\{g_{10}^{\alpha}(\tau)|\alpha\in\{0,0.2,\cdots,0.8,1\}\}$ are presented in Table \textbf{??}. The integration limits of the aformentioned Cavalieri 
...
be interpreted as an infinite sum of non-rectanguar infinitesimals whose shape is determined by its lower integration limit $a_t^{\alpha}(y)$. The list of functions 
$\{a_{10}^{\alpha}(y)|\alpha\in\{0,0.2,\cdots,0.8,1\}\}$ that can be associated with the integrals in Eq~{??} and Eq~{??} are presented in Table~\textbf{??}. Note that 
...
Each element of $\mathcal{C}$ can be associated with a particular choice of $t$.  
