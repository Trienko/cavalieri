...
\end{equation}
We can assign a geometric interpretation to the above fractional integral by converting it into a Cavalieri integral using two steps. First, we convert 
...
\begin{equation}
\int_{a_{10}^{\frac{4}{5}}(y)}^{b_t^{\alpha}(y)} f(\tau)~d\tau. 
\end{equation}
This integral can be interpreted as the area of the region bounded by the $x$-axis, the function $f(y)$ and the function $b_{10}^{\frac{4}{5}}(y)$. Let us denote 
this region with the symbol $R_1$. The region $R_1$ is depicted and labelled in the left most panel of Figure.~\ref{??}. The function    
