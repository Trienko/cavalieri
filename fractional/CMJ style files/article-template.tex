...
\noindent
Since, Eq.~\eqref{eq:cav} is a Cavalieri integral it has a clear geometric interpretation (see Fig.~\ref{fig:caval2}). The integral in Eq.~\eqref{eq:RL_frac_int} can, therefore, be interpreted as the area obtained 
by summing together the area of an infinite number of infinitesimally small non–rectangular integration strips whose shape is determined by $\alpha$ and $t$. The shape of theses integration 
strips will change if either $\alpha$ or $t$ is changed. If $\alpha$ is equal to one, however, then the integral reduces to a normal Riemann integral as the integration strips become rectangular for this particular choice of $\alpha$ (its shape is now also no longer affected by $t$).
...
number of infinitesimals whose shape is determined by $\alpha$ and $t$, i.e. the aforementioned infinitesimals have to be translations of $b_{\alpha}^t(y)$ which, in turn, is 
...
The thing which remains unclear, however, is in what way does $\alpha$ and $t$ affect the shape of the aforementioned infinitesimals, i.e. how does the value of $\alpha$ and $t$ affect the shape of $b_{\alpha}^t(y)$. 
We will answer this question using two steps. First off, we will fix the value of $alpha$ and $t$ in Eq.~\eqref{} and then depict $b_{\alpha}^t(y)$. We will then explore how varying the value of $\alpha$ and $t$ in Eq.~\eqref{} affects the shape $b_{\alpha}^t(y)$.
...
Moreover, let $A_{10}^{\sfrac{4}{5}}$ denote the area of $R_{10}^{\sfrac{4}{5}}$. For the sake of completeness, lets first verify whether we can emplloy the Cavalieri integral in Eq.~\ref{eq:ex1_specific} to compute $A_{10}^{\sfrac{4}{5}}$ from first principles.
...
construct a so called Cavalieri sum. Furthermore, the sides of the non-rectangular integration strips we use in this approximation have to be translations of $b_{10}^{\sfrac{4}{5}}(y)$.
If we take the limit of the aforementioned sum as $n\rightarrow \infty$ we obtain $A_{10}^{\sfrac{4}{5}}$, i.e. the result obtained via the aforementioned limit is the Cavalieri integral found in Eq.~\eqref{eq:ex1_specific}. In conclusion, we can think of Eq.~\eqref{} as implying that the region $R_{10}^{\sfrac{4}{5}}$ is made up 
of an infinite number of non-rectangular infinitesimals; all of them being translations of $b_{10}^{\sfrac{4}{5}}(y)$. Note, the rightmost curve depicted in the rightmost panel of Fig... is $b_{10}^{\sfrac{4}{5}}(y)$.  Moreover, the special case in which we approximate $A_{10}^{\sfrac{4}{5}}$ using five equal width non-rectangular integration strips is depicted in the right-most panel of Figure~\ref{??}. 
...
the larger the $t$-value is, that is assoiciated with it.
...
\end{description}
We can therefore draw the following conclusion: the value of both $\alpha$ and $t$ alters the shape of the infinitesimals (i.e. $b_{t}^{\alpha}(y)$) that make up the regions in Fig~\ref{??}. Interestingly, the effect of $\alpha$ on the shape of the infinitesimals that make up the regions depicted in Fig~\ref{??} is larger than the effect that $t$ has on their shape. 
