...
The sides of the aforementioned five integration strips are depicted in Fig.~\ref{fig:geo1} using dashed lines.\\ 
...
Let $R_{t}^{\alpha}$ denote the region which is bounded by the $\tau$-axis, the function $f(\tau)$ and the function $b_{t}^{\alpha}(y)$. Moreover, let $A_{t}^{\alpha}$ denote the area of $R_{t}^{\alpha}$ and let $\mathsf{R}_{\mathsf{T}}^{\mathsf{A}}=\{R_t^{\alpha}|t\in\mathsf{T},~\alpha\in\mathsf{A}\}$  with $\mathsf{T}=\{2,4,6,8,10\}$. 
...
\noindent
As was the case for $R_{10}^{\sfrac{4}{5}}$, Eq.~\eqref{eq:ex1} implies that we can assume that each of the regions depicted in Fig.~\ref{fig:geo1} are made up of an infinite number of infinitesimals. The infinitesimals that make up a specific region are translations of that region's right edge. 
...
\end{description}
We can therefore draw the following conclusion: the value of both $\alpha$ and $t$ alters the shape of the infinitesimals (i.e. the upper limit $b_{t}^{\alpha}(y)$ of Eq.~\eqref{eq:ex1} depends on $\alpha$ and $t$) that make up the regions in Fig~\ref{??}. Interestingly, the effect of $\alpha$ on the shape of the infinitesimals that make up the regions depicted in Fig~\ref{??} is larger than the effect that $t$ has on their shape. 
