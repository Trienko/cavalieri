...
\section{Examples}
We use the geometric framework we presented in the previous paragraph to provide 
To make things clear we provide two fully worked examples below of how to geometrically interpret fractional integrals using conversion  presented in the previsous 
section. The two fractional integrals we will visualize are:\\ 

\noindent
$\frac{1}{\Gamma(\alpha)}\int_0^t \tau(t-\tau)^{\alpha-1}~d\tau$ and $\frac{1}{\Gamma(\alpha)}\int_0^t \sqrt{\tau}(t-\tau)^{\alpha-1}~d\tau$.\\

The steps to visualize the above two integral wer described in the previous paragraph. We first convert the integrals into Riemann-Stieltjes integrals. The 
Riemann-Stieltjes integrals are then converted to Cavalieri integrals. The graphical  
