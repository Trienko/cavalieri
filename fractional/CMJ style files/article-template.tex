...
Moreover, let $A_{10}^{\sfrac{4}{5}}$ denote the area of $R_{10}^{\sfrac{4}{5}}$. We are now in a position to assign a geometric interpretation to Eq.~\eqref{eq:ex1_specific}.  
We can approximate the area of $A_{10}^{\sfrac{4}{5}}$ by summing together the area of $n$ equal width non-rectangular integration strips that inscribe $R_{10}^{\sfrac{4}{5}}$. 
Furthermore, the sides of these non-rectangular integration strips have to be translations of $b_{10}^{\sfrac{4}{5}}(y)$.
If we take the limit of the aforementioned sum as $n\rightarrow \infty$ we obtain $A_{10}^{\sfrac{4}{5}}$, i.e. the region $R_{10}^{\sfrac{4}{5}}$ is made up 
of an infinite number of infinitesimals; all of them being translations of $b_{10}^{\sfrac{4}{5}}(y)$. The special case in which we approximate $A_{10}^{\sfrac{4}{5}}$ using five equal width non-rectangular integration strips is depicted in the right-most panel of Figure~\ref{??}. 
...
Varying both $\alpha$ and $t$ results in Eq.~\eqref{eq:ex1}. In contrast with Eq.~\eqref{eq:ex1_specific}, Eq.~\eqref{eq:ex1} can be associated with a multitude of regions. 
...
\begin{description}
 \item[Effect of $\alpha$] the grey-tinted solid curves depicted in each panel are not equal to one-another.
 \item[Effect of $t$] the dashed grey curves and the solid grey-tinted curves in the right-most panel are not equal to one-another.
\end{description}
...
If we evaluate the integral in Eq for all $\alpha\in \mathsf{A}$ we obtain the curves in the left-most column of Table.... These curves are depicted in Fig.
The area of the regions depicted in Fig are also plotted in Fig using circular markers. As expected, the circular markers that correspond to the areas of the regions 
from a particular choice of $\alpha$ (i.e. from a particular panel of Fig) in Fig lie on the curve in Fig that is associated with the same choice of $alpha$. Moreover, note that we have adopted the 
same color scheme in Fig and Fig. Using the same color scheme in both Figures makes it clear which of the area values in Fig can be associated with which regions in Fig.
