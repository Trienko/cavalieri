...
converting the above integrals into Riemann-Stieltjes integrals and then converting the resulting integrals into Cavalieri integrals. We can convert fractional 
integral into Riemann-Stieltjes integrals by utilizing Eq.~\eqref{}. Riemann
The inegrator $g_t^{\alpha}(\tau)$ of the aforementioned intermediate Riemann-Stieltjes integrals are equal and independent of the integrand $f(\tau)$.
The set of functions $\{g_{10}^{\alpha}(\tau)|\alpha\in\{0,0.2,\cdots,0.8,1\}\}$ are presented in Table \textbf{??}.  
